...
### What this is
This repo is the **BAD006** batch job (`Get-DcsLoginLogoutLogs`): a Windows **PowerShell** job that exports `SYS_USER_LOGIN_TBL` rows from a SQL Server by `LAST_UPD_DT` range, writes CSV and login/logout TXT outputs, zips them, and uploads the zip via SFTP. It is not a service — it is a one-shot script. Files: `BAD006.ps1` (entry point), `util.ps1` (functions), `config.ps1` (paths/connection/SFTP settings), `README.md`.
...
The runnable core is `SqlExportToCSV`, `SqlExportLoginOutText`, and `ZipFiles` from `util.ps1`. To test without touching prod:
...
3. In a harness script: dot-source `util.ps1`, set `$connectionString` to the local server (`Server=localhost,1433;...;TrustServerCertificate=True;`), set `$logFile` (required by `Log`), then call `SqlExportToCSV <sql> <csvPath>`, `SqlExportLoginOutText <sql> <txtPath>`, and `ZipFiles @(<csvPath>, <txtPath>) <zipPath>`. Do not edit `config.ps1`/`util.ps1` — override the variables in the harness instead.
